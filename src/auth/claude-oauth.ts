...
export namespace ClaudeOAuth {
  ...
  export async function saveState(state: OAuthState): Promise<void> {
    ...
    log.info("saved oauth state", { expiresAt: new Date(state.expiresAt).toISOString() })
  }
   ...
   */
  export async function getCredentials(): Promise<Credentials["claudeAiOauth"] | undefined> {
    try {
      const file = Bun.file(credentialsPath)
      if (!(await file.exists())) {
