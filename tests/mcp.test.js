...
describe('MCP CLI Commands', () => {
  ...
  test('mcp --help shows available commands', async () => {
    const result = await $`XDG_CONFIG_HOME=${testConfigDir} bun run ${projectRoot}/src/index.js mcp --help`.quiet().nothrow()
  ...
  test('mcp add --help shows usage', async () => {
    const result = await $`XDG_CONFIG_HOME=${testConfigDir} bun run ${projectRoot}/src/index.js mcp add --help`.quiet().nothrow()
    const output = result.stdout.toString()
  ...
  test('mcp add creates correct config for Playwright MCP', async () => {
    ...
    const result = await $`XDG_CONFIG_HOME=${testConfigDir} bun run ${projectRoot}/src/index.js mcp add playwright npx @playwright/mcp@latest`.quiet().nothrow()
    ...
    const configContent = await fs.readFile(path.join(configDir, 'opencode.json'), 'utf-8')
    ...
    expect(config.mcp.playwright.command).toEqual(['npx', '@playwright/mcp@latest'])
  ...
  test('mcp add supports multiple arguments for command', async () => {
    ...
    const result = await $`XDG_CONFIG_HOME=${testConfigDir} bun run ${projectRoot}/src/index.js mcp add test-mcp node /path/to/server.js arg1 arg2`.quiet().nothrow()
    ...
    expect(config.mcp['test-mcp'].command).toEqual(['node', '/path/to/server.js', 'arg1', 'arg2'])
  })
  ...
  test('mcp add with --enabled=false creates disabled server', async () => {
    ...
    const result = await $`XDG_CONFIG_HOME=${testConfigDir} bun run ${projectRoot}/src/index.js mcp add disabled-mcp npx @test/mcp@latest --enabled=false`.quiet().nothrow()
  ...
  test('mcp add with --url creates remote server config', async () => {
    ...
    const result = await $`XDG_CONFIG_HOME=${testConfigDir} bun run ${projectRoot}/src/index.js mcp add remote-server --url https://example.com/mcp`.quiet().nothrow()
  ...
  test('mcp list shows configured servers', async () => {
    ...
    const result = await $`XDG_CONFIG_HOME=${testConfigDir} bun run ${projectRoot}/src/index.js mcp list`.quiet().nothrow()
    const stderr = result.stderr.toString()
  ...
  test('mcp add preserves existing config', async () => {
    ...
    expect(config.mcp['existing-server'].command).toEqual(['npx', '@existing/mcp'])
...
describe('Playwright MCP Configuration Format', () => {
  ...
  test('config matches expected OpenCode format', async () => {
    ...
    await $`XDG_CONFIG_HOME=${testConfigDir} bun run ${projectRoot}/src/index.js mcp add playwright npx @playwright/mcp@latest`.quiet().nothrow()
