...
- 8feb57c: fix: accept null Claude OAuth subscription metadata
...
- cfb9811: fix: session summaries and compaction follow `--model`
...
- 15e5431: fix: a failed session summary no longer fails the run
...
- cea21b7: fix: share Git objects and prune orphaned snapshot stores
...
- 4bc4332: feat: emit `model_resolved`, a machine-readable model routing event
...
- 133dcd2: fix: exit non-zero on fatal startup errors and stop retrying refused connections
...
- 79ea855: Fix false positives, false negatives, warnings and errors in CI/CD (#287):
...
- 9f57349: Fix three defects that made a session's behaviour unattributable (#285):
...
- 1bd2427: Add a native, enforceable permission system, ported from OpenCode and fully controllable over JSON with no TUI (issue #271).
